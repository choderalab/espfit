...
@pytest.fixture
def test_create_test_espaloma_system():
    ...
    c = SetupSampler(small_molecule_forcefield=ESPALOMA_FORCEFIELD)
    c.create_system(biopolymer_file=biopolymer_file)

    return c


def test_create_nucleoside_espaloma_system():
...
def test_create_protein_ligand_espaloma_system():
...
def test_create_protein_ligand_openff_system():
    ...
    # Convert PosixPath to str to avoid error when loading molecule with RDKIT.
    ligand_file = str(files('espfit').joinpath('data/target/testsystems/protein-ligand/ligands.sdf'))
    c = SetupSampler(small_molecule_forcefield='openff-2.1.0')
    c.create_system(biopolymer_file=biopolymer_file, ligand_file=ligand_file)


@pytest.mark.skip("Time demanding test. Skip to speedup the test suite.")
def test_create_protein_espalom_system():
...
@pytest.mark.skip("Time demanding test. Skip to speedup the test suite.")
def test_create_multi_protein_ligand_espaloma_system():
...
def test_export_system_change_outdir(test_create_test_espaloma_system, tmpdir):
    ...
    c.export_xml(output_directory_path=str(tmpdir))
